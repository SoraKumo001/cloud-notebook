...
`.dev.vars`(ローカル開発用 env ファイル)は `pnpm setup:dev` 実行時に `.dev.vars.example` から自動コピーされます(冪等:既存ファイルは上書きしません)。ダミー値の `SESSION_SECRET` / `API_KEY_ENCRYPTION_MASTER` を含むため、追加編集なしで `wrangler dev` が起動可能です。開発モードでは `authMiddleware` がダミーユーザー(`dev-user@example.com`)を返すため、認証なしで全機能が試せます。
