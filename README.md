...
`.dev.vars`(ローカル開発用 env ファイル)は `pnpm setup:dev` 実行時に `.dev.vars.example` から自動コピーされます(冪等:既存ファイルは上書きしません)。ダミー値の `SESSION_SECRET` / `API_KEY_ENCRYPTION_MASTER` を含むため、追加編集なしで `wrangler dev` が起動可能です。開発モードでは email + password 認証が動作します。`pnpm dev` 起動後、`/api/auth/register` で最初のユーザーを登録すると自動的に管理者になります。以降はそのユーザーでログインしてご利用ください。`SESSION_SECRET` が `.dev.vars` に設定されていることを確認してください。

### 開発環境セットアップ(初回のみ)

Vectorize index(`cloud-notebook-vector-bge-dev`)のみ Cloudflare 上に作成します。D1 と R2 はローカルエミュレータ(miniflare)を使用するため、Cloudflare 上に DB やバケットは作成されません(無料枠の消費なし)。ローカル D1 には SQLite ファイルとしてマイグレーションが自動適用されます。

```bash
pnpm setup:dev         # ルートから(workspace 経由)
# または
pnpm --filter backend setup:dev
```

この 1 コマンドで以下が冪等に実行されます:

| Step | 内容 |
|---|---|---|
| 0. Env | `.dev.vars` が無ければ `.dev.vars.example` からコピー(既存は上書きしない) |
| 1. Config | `wrangler.jsonc` を dev テンプレに正規化(必要なら `database_id` をプレースホルダに、`database_name` / `bucket_name` を prod 名に書き戻し) |
| 2. Vectorize | `cloud-notebook-vector-bge-dev` (1024-dim cosine) がなければ作成 + `notebook_id` / `source_id` metadata index を保証 |
| 3. Migrations | `wrangler d1 migrations apply DB --local`(ローカル SQLite に適用) |

metadata index は `notebook_id`(string) と `source_id`(string) の 2 つです。これらはチャット時のノートブックフィルタリングとソース単位の検索に使用されます。

> 個別に手動実行する場合: `pnpm --filter backend vectorize:create-dev` → `pnpm --filter backend vectorize:create-metadata-indexes-dev` → `pnpm --filter backend db:migrate:local`
...
> **課金注意**: D1 と R2 は wrangler dev のローカルエミュレーション(miniflare / SQLite / ローカルファイルシステム)で動作するため、Cloudflare にリクエストを送らず無料枠を消費しません。Vectorize のみリモート接続(無料枠内)で動作し、本番 `cloud-notebook-vector-bge` のデータには触れません。`vectorize` は `wrangler.jsonc` で `remote: true` が明示されています。大量データの upsert や LLM 呼び出しを繰り返すと Cloudflare の無料枠を超えて課金が発生します。詳細は [`packages/backend/docs/deployment.md`](packages/backend/docs/deployment.md) の「ローカル開発の課金リスク」を参照。
...
# テスト
pnpm test                        # 両方 (vitest)
pnpm --filter backend test       # 266 tests
pnpm --filter frontend test      # 163 tests
...
- ci.yml (push/PR to master): lint + format + Vitest テスト + ビルド。本番デプロイは `pnpm deploy:full` を実行 (詳細は [packages/backend/docs/deployment.md](packages/backend/docs/deployment.md))。
